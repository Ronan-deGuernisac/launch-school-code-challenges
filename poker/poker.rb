...
#   Values of cards in each hand of the same rank are compared in order until one hand
#   is found to have a card of higher value than another hand. If all cards are of equal value
...
class Poker
  ...
  def best_hand
    ranked_hands = rank_hands
    # binding.pry
    return sanitize_output(ranked_hands) if ranked_hands.size == 1
    sanitize_output(rank_cards(ranked_hands))
  end
  ...
  def rank_hands
    best_rank = @hands.map { |hand| hand.rank }.min
    @hands.reject { |hand| hand.rank != best_rank }
  end
  
  def rank_cards(ranked_hands)
    sorted_valued_hands = ranked_hands.map { |hand| hand.comparison_order }
    
    hands_ranked_by_card_values = nil
    5.times do
      hands_ranked_by_card_values = []
      comparison_cards = {}
      sorted_valued_hands.each_with_index do |hand, index|
        comparison_cards[index] = hand.last
        hand.rotate!
      end
      highest_value = comparison_cards.map { |card_key, card_value| card_value.value }.max
      comparison_cards.reject! { |card_key, card_value| card_value.value < highest_value }
      comparison_cards.keys.each { |hand_index| hands_ranked_by_card_values << ranked_hands[hand_index] }
...
class PokerHand
  ...
  def order_cards
    ordered_cards = @cards.sort_by { |card| card.value }
